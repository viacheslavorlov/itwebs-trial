      ...
      <div className={`mt-4 flex justify-center space-x-2 ${className}`}>
        {items.map((_, index) => (
          <button
            ...
            className={`h-3 w-3 rounded-full transition-all duration-300 focus:outline-none ${current === index
...
export function AutoplayCarousel({
  ...
  const defaultRenderItem = (item: Character, index: number) => (
    <ItemComponent
      character={item}
      id={item.id}
...
export const ItemComponent = React.memo(
  ({ character }: { id: number, isActive: boolean, character: Character }) => (
    <div
